...
public class Game : MonoBehaviour
{
    ...
    void GameOver()
    {
        PlayerPrefs.SetInt("Combo", OrderShow.Ins._Combo);
        PlayerPrefs.SetInt("Money", OrderShow.Ins._Money);

        PlayerPrefs.SetInt("TopScore1", OrderShow.Ins._clear);
        if (OrderShow.Ins._clear >= OrderShow.Ins._BESTordernum)
        {
            OrderShow.Ins._BESTordernum = OrderShow.Ins._clear;
            PlayerPrefs.SetInt("TopScore", OrderShow.Ins._BESTordernum);
        }

        OrderShow.Ins._ORDERNUM.text = "" + OrderShow.Ins._ordernum.ToString();
        OrderShow.Ins._BESTORDERNUM.text = "" + OrderShow.Ins._BESTordernum.ToString();
        OrderShow.Ins._Reward.text = "" + string.Format("{0:#,###0}", OrderShow.Ins._Money.ToString());
